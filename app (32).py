"""
High-Pressure Gas Flow Calculator  —  v3
=========================================
Changes from app(29).py:
  1. hud_background.html loaded as base64 iframe background (no custom HTML panels)
  2. Full CSS-variable color theming — ALL text changes color in orange/red modes
  3. ** operator replaced with Math.pow() for cross-browser compatibility
  4. Safety limits: CO2>70, CH4>250, C2H2>17, H2S>20, ANY gas>2001 bar → BLOCKED (red)
  5. Dangerous gases O2/H2/CH4/C2H2/H2S → orange theme
  6. 2-column form layout activates in warning/danger modes (matches images 9 & 10)
  7. RESULTS box + Recommendation Confidence bar added
...
/* ── CSS theme variables (default = teal) ───────────────────────────────── */
:root{
  --tc :#00e5cc; --tc2:#00ffee; --tc3:rgba(0,229,204,0.40);
  --tc4:rgba(0,229,204,0.22); --lc :rgba(0,229,204,0.75);
  --pbg:rgba(0,8,8,0.96);     --ibg:rgba(0,14,12,0.98);
  --bbg:rgba(0,229,204,0.07); --bh :rgba(0,229,204,0.20);
  --gs :rgba(0,229,204,0.28); --ge :rgba(0,229,204,0.72);
  --rbg:rgba(0,229,204,0.06); --hbg:rgba(0,14,12,0.95);
}

/* ── Orange: dangerous gas ──────────────────────────────────────────────── */
.warn{
  --tc :#ff8c00; --tc2:#ffcc66; --tc3:rgba(255,140,0,0.50);
  --tc4:rgba(255,140,0,0.25);  --lc :rgba(255,190,60,0.85);
  --pbg:rgba(20,8,0,0.97);     --ibg:rgba(28,11,0,0.99);
  --bbg:rgba(255,140,0,0.07);  --bh :rgba(255,140,0,0.22);
  --gs :rgba(255,140,0,0.30);  --ge :rgba(255,140,0,0.80);
  --rbg:rgba(255,140,0,0.05);  --hbg:rgba(28,11,0,0.97);
}

/* ── Red: pressure limit blocked ────────────────────────────────────────── */
.dngr{
  --tc :#ff2222; --tc2:#ff7777; --tc3:rgba(255,40,40,0.50);
  --tc4:rgba(255,40,40,0.25);  --lc :rgba(255,120,120,0.85);
  --pbg:rgba(20,0,0,0.98);     --ibg:rgba(26,2,2,0.99);
  --bbg:rgba(255,40,40,0.07);  --bh :rgba(255,40,40,0.22);
  --gs :rgba(255,40,40,0.30);  --ge :rgba(255,40,40,0.85);
  --rbg:rgba(255,40,40,0.05);  --hbg:rgba(26,2,2,0.97);
}
...
#panel{
  pointer-events:all;
  width:min(97%,580px);max-height:97vh;overflow:hidden;
  background:var(--pbg);border:1.5px solid var(--tc);border-radius:4px;
  padding:12px 22px 16px;position:relative;
  animation:pglow 2.5s ease-in-out infinite alternate;
  transition:background 0.35s,border-color 0.35s;
  color:var(--tc);
}
@keyframes pglow{
  from{box-shadow:0 0 8px var(--gs),inset 0 0 24px rgba(0,0,0,0.4)}
  to  {box-shadow:0 0 36px var(--ge),inset 0 0 50px rgba(0,0,0,0.2)}}
...
@keyframes ahpls{from{box-shadow:0 0 4px var(--gs)}to{box-shadow:0 0 20px var(--ge)}}
#alert-hdr .ah-icon{font-size:16px;color:var(--tc)}
#alert-hdr .ah-text{
  font-size:12px;font-weight:bold;letter-spacing:2.5px;text-transform:uppercase;
  color:var(--tc);text-shadow:0 0 10px var(--gs);
}

/* ── Title ──────────────────────────────────────────────────────────────── */
#panel h1{
  position:relative;z-index:1;color:var(--tc2);
  text-shadow:0 0 14px var(--gs);
...
/* 2-column mode — activated by .warn or .dngr on #panel */
.warn .sr,.dngr .sr{flex-direction:row;align-items:center;gap:10px}
.warn .sr label,.dngr .sr label{min-width:48%;max-width:48%;font-size:12px}
.warn .sr select,.dngr .sr select{flex:1}

/* ── Divider ─────────────────────────────────────────────────────────────── */
.hdiv{border:none;border-top:1px solid var(--tc4);margin:2px 0;
  transition:border-color 0.35s}

/* ── Input fields (stacked by default) ─────────────────────────────────── */
#ifields{display:flex;flex-direction:column;gap:5px}
.frow{display:flex;flex-direction:column;gap:2px}
.frow label{font-size:13px;color:var(--lc);letter-spacing:0.2px;
...
/* 2-column fields for warn/danger */
.warn .frow,.dngr .frow{flex-direction:row;align-items:center;gap:10px}
.warn .frow label,.dngr .frow label{min-width:48%;max-width:48%}
.warn .frow input,.dngr .frow input{flex:1}

/* ── Caption ─────────────────────────────────────────────────────────────── */
.cap{font-size:11px;color:var(--lc);opacity:0.5;letter-spacing:0.8px}
...
#cbtn{
  ...
  transition:background 0.18s,box-shadow 0.18s,border-color 0.35s,color 0.35s}
#cbtn:hover{background:var(--bh);box-shadow:0 0 18px var(--ge)}
#cbtn:active{filter:brightness(1.3)}
/* Blocked state */
.dngr #cbtn{opacity:0.45;cursor:not-allowed;pointer-events:none}

/* ── Results ─────────────────────────────────────────────────────────────── */
.rbox{padding:9px 13px;border-radius:2px;font-size:13px;line-height:1.7;word-break:break-word}
.rbox.ok{
  background:rgba(0,52,26,0.94);border:1px solid rgba(0,200,100,0.40);
  border-left:3px solid #00e87a;color:#00ffcc}
.rbox.ok .res-title{font-size:15px;font-weight:bold}
.rbox.warn2{
  background:rgba(50,22,0,0.95);border:1px solid rgba(255,175,35,0.38);
  border-left:3px solid #ff8c00;color:#ffcc77}
.rbox.blk{
  background:rgba(48,0,0,0.97);border:1px solid rgba(255,40,40,0.40);
  border-left:3px solid #ff2222;color:#ff7777;font-weight:bold;
  animation:bpls 1.4s ease-in-out infinite alternate}
@keyframes bpls{
  from{box-shadow:0 0 4px rgba(255,40,40,0.18)}
  to  {box-shadow:0 0 14px rgba(255,40,40,0.50)}}
.rbox.err{
  background:rgba(36,0,0,0.95);border:1px solid rgba(255,50,50,0.30);
  border-left:3px solid #ff4444;color:#ff9090}

/* RESULTS wrapper box (warning/danger mode) */
#results-section{
  border:1px solid var(--tc4);border-radius:2px;
  background:var(--hbg);padding:9px 13px;display:none;
  transition:border-color 0.35s,background 0.35s}
.warn #results-section,.dngr #results-section{display:block}
.results-lbl{
  font-size:10px;letter-spacing:2.5px;text-transform:uppercase;
  color:var(--lc);margin-bottom:6px;opacity:0.75}

/* ── Spec card ───────────────────────────────────────────────────────────── */
.spec-card{
  margin-top:7px;padding:9px 13px;border-radius:2px;
  background:rgba(0,26,20,0.98);
  border:1px solid var(--tc4);border-left:3px solid var(--tc);
  transition:border-color 0.35s}
.sc-head{color:var(--lc);opacity:0.6;font-size:10px;letter-spacing:2px;
  text-transform:uppercase;margin-bottom:6px}
.sc-row{color:var(--tc2);font-size:14px;line-height:1.95}
.sc-note{color:var(--lc);opacity:0.5;font-size:11px;margin-top:3px}
.badge{display:inline-block;border-radius:2px;font-size:9px;
...
.warn #conf-section,.dngr #conf-section{display:block}
...
.conf-fill{
  height:100%;background:var(--tc);position:absolute;left:0;top:0;
  transition:width 0.7s ease,background 0.35s;
  display:flex;align-items:center;justify-content:flex-end;padding-right:3px}
/* Segmented look: notches every ~10% via repeating gradient */
.conf-fill::before{
  content:'';position:absolute;inset:0;
  background:repeating-linear-gradient(90deg,
    transparent 0,transparent calc(10% - 1px),
    rgba(0,0,0,0.35) calc(10% - 1px),rgba(0,0,0,0.35) 10%)}
.conf-pct{font-size:12px;font-weight:bold;color:var(--tc);
  min-width:38px;text-align:right;transition:color 0.35s}
</style>
</head>
<body>
...
<div id="panel">
  ...
  <!-- Alert header (hidden in normal mode, shown in warn/danger) -->
  <div id="alert-hdr">
  ...
  <div id="body">
    ...
    <!-- Normal mode result (shown outside results-section) -->
    <div id="result-normal"></div>

    <!-- Warning/Danger mode: RESULTS section -->
    <div id="results-section">
      <div class="results-lbl">Results</div>
      <div id="result-wd"></div>
    </div>

    <!-- Confidence bar (warn/danger modes) -->
    <div id="conf-section">
      <div class="conf-row">
...
<script>
// ── TUBE TABLE (injected from Python) ────────────────────────────────────────
var TT = %%TUBE_TABLE%%;

// ── Safety limits that BLOCK calculation (red state) ────────────────────────
// Any gas over 2001 bar is also blocked.
var BLK_LIMITS = {
  "CO2" :{max:69.9,
    hdr:"ALERT: SAFETY / LIMIT CONDITION",
...
var GLOBAL_MAX = 2001; // all gases blocked above this

// Gases that trigger orange warning (but not blocked unless pressure exceeded)
var WARN_GASES = ["O2","H2","CH4","C2H2","H2S"];

var WARN_MSGS = {
  "O2" :"O\u2082 \u2014 Oxidizing gas. Use S14 O\u2082-cleaned equipment only.",
  "H2" :"H\u2082 \u2014 Highly flammable. Use S6 (\u2264200 bar) or S9 (\u2264400 bar) only.",
  "CH4":"CH\u2084 \u2014 Flammable. Max 250 bar. Use S16 double-contained spec.",
  "C2H2":"C\u2082H\u2082 \u2014 Unstable above 17 bar. Decomposition risk!",
  "H2S":"H\u2082S \u2014 Toxic & flammable. Max 20 bar. Use PPE and gas detection."
};

// ── Theme engine ──────────────────────────────────────────────────────────────
function setMode(m, hdrTxt) {
  ...
  if (at && hdrTxt) at.textContent = hdrTxt;
}

function evalMode(gas, Pi) {
  // 1) Global pressure cap — all gases
  if (Pi > GLOBAL_MAX) {
    return {m:"D", hdr:"ALERT: SAFETY / LIMIT CONDITION",
      blocked:true, blkMsg:"Risk: Pressure Exceeds Maximum Limit!\n>2001 bar is not permitted for any gas."};
  }
  // 2) Gas-specific pressure block
  var lim = BLK_LIMITS[gas];
  if (lim && Pi > lim.max) {
    return {m:"D", hdr:lim.hdr, blocked:true, blkMsg:lim.msg};
  }
  // 3) Orange warning for dangerous gas
  if (WARN_GASES.indexOf(gas) !== -1) {
    return {m:"W", hdr:"WARNING: PRESSURE LIMIT REACHED", blocked:false};
  }
  return {m:"N", blocked:false};
}

// ── Field key (ORIGINAL logic from app.py — parens removed, not replaced) ────
function toKey(f) {
...
function buildSpecCard(gas, inP, diam) {
  ...
  if (!m) return "<div class='rbox warn2'><strong>\u26a0 No approved tube spec found</strong></div>";
  var badge="";
...
// Compact result for warn/danger mode (matches images 9 & 10)
function buildCompactResult(gas, inP, diam) {
  var m = lookupTube(gas, inP, diam);
  if (!m) return "No approved tube specification found.";
  return "Required Diameter: <strong>"+diam.toFixed(2)+" mm</strong><br>"
    +"Recommended Spec: <strong>"+m.spec+"</strong><br>"
    +"Tube: <strong>"+fmtTube(m.tube_od_w)+"</strong>"
...
function calcConf(gas, inP, diam, spec) {
  ...
  var sf = WARN_GASES.indexOf(gas) !== -1 ? 0.72 : 1.0;
  return Math.max(5, Math.min(100, Math.round((dr*0.35 + pr*0.65)*sf*100)));
}

function showConf(pct) {
  var f=document.getElementById("conf-fill"), l=document.getElementById("conf-pct");
  setTimeout(function(){f.style.width=pct+"%"; l.textContent=pct+"%";}, 80);
}

// ── Gas physics (UNCHANGED, Math.pow instead of ** operator) ─────────────────
...
function doCalc() {
  ...
  // Update theme from current state
  setMode(th.m, th.hdr);

  var rNorm = document.getElementById("result-normal");
  var rWD   = document.getElementById("result-wd");

  // BLOCKED: show safety message
  if (th.blocked && ct !== "inlet") {
    var bMsg = th.blkMsg.replace(/\n/g,"<br>");
    rNorm.innerHTML = "";
    rWD.innerHTML   = "<div class='rbox blk'>\u26a0\ufe0f SAFETY BLOCKED<br>"
      +"<span style='font-weight:normal;font-size:12px'>"+bMsg+"</span></div>";
    showConf(5);
    return;
  }
  ...
  try {
    ...
    var diam, inP, resultLine, html;

    if (ct === "diameter") {
      var r = calcDiameter(Pi,Po,Tc,L,Q,gas);
      diam = r; inP = Pi;
      resultLine = "Required Diameter: <strong>"+r.toFixed(2)+" mm</strong>";
    } else if (ct === "flow") {
      var r = calcFlow(Pi,Po,Tc,L,D,gas);
      diam = D; inP = Pi;
      resultLine = "Maximum Flow Rate: <strong>"+r.toFixed(1)+" L/min</strong>";
    } else if (ct === "length") {
      var r = calcLength(Pi,Po,Tc,D,Q,gas);
      diam = D; inP = Pi;
      resultLine = "Maximum Pipe Length: <strong>"+r.toFixed(1)+" m</strong>";
    } else if (ct === "inlet") {
      var r = calcInlet(Po,Tc,L,D,Q,gas);
      diam = D; inP = r;
      // Re-evaluate mode after solving for inlet
      var th2 = evalMode(gas, r);
      setMode(th2.m, th2.hdr);
      if (th2.blocked) {
        rNorm.innerHTML = "";
        rWD.innerHTML = "<div class='rbox ok'><div class='res-title'>Required Inlet: <strong>"+r.toFixed(2)+" bar</strong></div></div>"
          +"<div class='rbox blk' style='margin-top:5px'>\u26a0\ufe0f SAFETY BLOCKED<br>"
          +"<span style='font-weight:normal;font-size:12px'>"+th2.blkMsg.replace(/\n/g,"<br>")+"</span></div>";
        showConf(10); return;
      }
      resultLine = "Required Inlet Pressure: <strong>"+r.toFixed(2)+" bar</strong>";
    } else { // outlet
      var r = calcOutlet(Pi,Tc,L,D,Q,gas);
      diam = D; inP = Pi;
      resultLine = "Estimated Outlet Pressure: <strong>"+r.toFixed(2)+" bar</strong>";
    }

    var spec = lookupTube(gas, inP, diam);
    var pct  = calcConf(gas, inP, diam, spec);

    // Normal mode: rich spec card layout
    if (th.m === "N") {
      rNorm.innerHTML = "<div class='rbox ok'><div class='res-title'>"+resultLine+"</div></div>"
        + buildSpecCard(gas, inP, diam);
      rWD.innerHTML = "";
    } else {
      // Warning/Danger mode: compact result in RESULTS box
      rNorm.innerHTML = "";
      rWD.innerHTML = "<div style='color:var(--tc2);font-size:13px;line-height:1.8'>"
        + resultLine + "<br>"
    ...
    document.getElementById("result-normal").innerHTML =
      "<div class='rbox err'>\u26a0 "+e.message+"</div>";
    document.getElementById("result-wd").innerHTML = "";
  }
}

// ── Field builder + live pressure listener ────────────────────────────────────
function rebuildFields() {
  ...
  // Update theme based on current gas + default pressure
  var Pi = ct === "inlet" ? 0 : (DEF["Inlet Pressure (bar)"] || 0);
  var th = evalMode(gas, Pi);
  setMode(th.m, th.hdr);

  // Show gas warning in results if dangerous gas selected (normal calculation)
  if (th.m === "W" && !th.blocked) {
    var msg = WARN_MSGS[gas] || "";
    document.getElementById("result-wd").innerHTML =
      "<div style='color:var(--tc2);font-size:12px;opacity:0.88'>"+msg+"</div>";
  }
}
...
function onPressureInput() {
  var gas = document.getElementById("gasSelect").value;
  var ct  = document.getElementById("calcSelect").value;
  if (ct === "inlet") return; // Pi is the unknown
  var Pi = getInletP();
  var th = evalMode(gas, Pi);
  setMode(th.m, th.hdr);
}
