...
<style>
  ...
  /* ── Kill ALL scroll everywhere in Streamlit's DOM ─────────────────────── */
  ...
  /* ── Stretch the iframe to cover full viewport ─────────────────────────── */
...
html,body{
  ...
  /* No scroll at the iframe level either */
  overflow:hidden;
  background:#010a08;
...
#wrapper{
  position:fixed;
  left:22%;       /* px(422)/1920 */
  right:39.7%;    /* 1 - px(1158)/1920 */
  top:5.6%;       /* py(58)/1037 */
  bottom:0;
  /* Center the panel vertically in the black zone when it is smaller */
  display:flex;
...
/* ── Panel: auto height, centered, never overflows the zone ─────────────── */
#panel{
  pointer-events:all;
  width:100%;
  /* Shrink to fit content; grow as results are added */
  height:auto;
  /* Hard cap: never push beyond the black zone boundaries */
...
/* ── Smooth fade-in for results (no layout jumps) ────────────────────────── */
.rbox,.spec-card,#res-box,#conf-wrap{
...
components.html(page_html, height=2, scrolling=False)
